...
export default function Compteur5() {
  ...
  useEffect(() => {
    axios
      .get(api() + "/Compteur5/allCompteur5")
      .then((res) => {
        const dataTrEne = res.data.compteur5.map(item => ({
          time: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar
        }));
        const dataTrFre = res.data.compteur5.map(item => ({
          minute: item.TimeStamp,
          frequency: item.Freq
        }));
        const dataTrPui = res.data.compteur5.map(item => ({
          minute: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar,
          apparent: item.Sva
        }));
        const dataTrTens = res.data.compteur5.map(item => ({
          minute: item.TimeStamp,
          VoltageU12: item.U12,
          VoltageU23: item.U23,
          VoltageU31: item.U31
        }));
        const dataTrCou = res.data.compteur5.map(item => ({
          minute: item.TimeStamp,
    ...
    socket.on("compteur5:last", (payload) => {
