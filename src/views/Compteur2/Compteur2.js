...
export default function Compteur2() {
  ...
  useEffect(() => {
    axios
      .get(api() + "/Compteur2/allCompteur2")
      .then((res) => {
        const dataTrEne = res.data.compteur2.map(item => ({
          time: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar
        }));
        const dataTrFre = res.data.compteur2.map(item => ({
          minute: item.TimeStamp,
          frequency: item.Freq
        }));
        const dataTrPui = res.data.compteur2.map(item => ({
          minute: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar,
          apparent: item.Sva
        }));
        const dataTrTens = res.data.compteur2.map(item => ({
          minute: item.TimeStamp,
          VoltageU12: item.U12,
          VoltageU23: item.U23,
          VoltageU31: item.U31
        }));
        const dataTrCou = res.data.compteur2.map(item => ({
          minute: item.TimeStamp,
    ...
    socket.on("compteur2:last", (payload) => {
