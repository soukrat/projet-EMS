...
export default function Compteur3() {
  ...
  useEffect(() => {
    axios
      .get(api() + "/Compteur3/allCompteur3")
      .then((res) => {
        const dataTrEne = res.data.compteur3.map(item => ({
          time: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar
        }));
        const dataTrFre = res.data.compteur3.map(item => ({
          minute: item.TimeStamp,
          frequency: item.Freq
        }));
        const dataTrPui = res.data.compteur3.map(item => ({
          minute: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar,
          apparent: item.Sva
        }));
        const dataTrTens = res.data.compteur3.map(item => ({
          minute: item.TimeStamp,
          VoltageU12: item.U12,
          VoltageU23: item.U23,
          VoltageU31: item.U31
        }));
        const dataTrCou = res.data.compteur3.map(item => ({
          minute: item.TimeStamp,
    ...
    socket.on("compteur3:last", (payload) => {
