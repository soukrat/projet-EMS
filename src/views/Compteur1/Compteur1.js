...
export default function Compteur1() {
  ...
  useEffect(() => {
    axios
      .get(api() + "/Compteur1/allCompteur1")
      .then((res) => {
        const dataTrEne = res.data.compteur1.map(item => ({
          time: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar
        }));
        const dataTrFre = res.data.compteur1.map(item => ({
          minute: item.TimeStamp,
          frequency: item.Freq
        }));
        const dataTrPui = res.data.compteur1.map(item => ({
          minute: item.TimeStamp,
          active: item.Pw,
          reactive: item.Qvar,
          apparent: item.Sva
        }));
        const dataTrTens = res.data.compteur1.map(item => ({
          minute: item.TimeStamp,
          VoltageU12: item.U12,
          VoltageU23: item.U23,
          VoltageU31: item.U31
        }));
        const dataTrCou = res.data.compteur1.map(item => ({
          minute: item.TimeStamp,
    ...
    socket.on("compteur1:last", (payload) => {
